...
export function ChatWindow({ currentUser, selectedUser, onBack }: ChatWindowProps) {
  ...
  const headerSubline = isRecipientTyping ? (
    <span className="text-xs font-medium flex items-center gap-1.5" style={{ color: "var(--t-typing-color)" }}>
      typing
      <span className="inline-flex gap-[3px] items-end mb-[1px]">
        {[0, 150, 300].map((d) => (
          <span key={d} className="w-[3px] h-[3px] rounded-full animate-bounce"
            style={{ background: "var(--t-typing-color)", animationDelay: `${d}ms` }} />
        ))}
      </span>
    </span>
  ) : (
    <span className={`text-xs ${selectedUser.online ? "text-green-500" : "text-muted-foreground"}`}>
      {selectedUser.online
        ? "● Online"
  ...
  return (
    <div className="flex flex-col h-full w-full relative" style={{ background: "var(--t-chat-window-bg)" }}>

      {/* ── Premium Header ── */}
      <div className="gradient-chat-header shrink-0 z-10 sticky top-0">
        <div className="h-16 flex items-center px-4 gap-3">
          <Button variant="ghost" size="icon" onClick={onBack}
            className="md:hidden h-9 w-9 text-muted-foreground hover:text-foreground rounded-xl mr-0.5">
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <AnimatePresence mode="wait">
                <motion.div key={isRecipientTyping ? "typing" : "status"}
          ...
          <Button variant="ghost" size="icon" onClick={searchOpen ? closeSearch : openSearch}
            className={`h-9 w-9 shrink-0 rounded-xl transition-all ${searchOpen ? "text-primary bg-primary/10" : "text-muted-foreground hover:bg-muted"}`}
        ...
        <AnimatePresence>
          {searchOpen && (
            <motion.div
              ...
              className="overflow-hidden" style={{ borderTop: "1px solid var(--t-divider)" }}
            >
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:p-6"
        style={{ background: "var(--t-chat-area-gradient)" }}>
                    ...
                    <div className="bubble-received rounded-2xl rounded-bl-sm px-4 py-3 flex items-center gap-[5px]">
                      {[0, 150, 300].map((d) => (
                        <span key={d} className="w-2 h-2 rounded-full animate-bounce"
                          style={{ background: "var(--t-typing-color)", opacity: 0.6, animationDelay: `${d}ms` }} />
                      ))}
                    </div>
                  </motion.div>
                )}
